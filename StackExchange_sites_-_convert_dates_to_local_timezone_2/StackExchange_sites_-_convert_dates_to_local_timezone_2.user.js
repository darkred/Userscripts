// ==UserScript==
// @name        StackExchange sites - convert dates to local timezone 2
// @namespace   darkred
...
function convertTitleDates() {
    ...
    for (var i = 0; i < dates.length; i++) {
        ...
        titles[i] = dates[i].title;           // initial values
        texts[i] = dates[i].innerHTML;           // initial values
        ...
        // if (moment(temp, format0, true).isValid()) {
            ...
            fixedTooltip = toTimeZone(temp, localTimezone);
            ...
            if (dates[i].innerHTML.match(regex3)) {              // the new navigation
                ...
                fixedDate = dateTextArray.join(' ');


                // dates[i].onmouseenter = function(){
                //     dates[i].title = fixedTooltip;
                //     // dates[i].setAttribute('title', fixedTooltip);
                //     dates[i].innerHTML = fixedDate;
                // };
                // dates[i].onmouseleave = function(){
                //     dates[i].title = titles[i];
                //     dates[i].innerHTML = texts[i];
                // };
                $(function() {
                    $(dates[i]).hover(function(e) {
                        $(this).title.html(fixedTooltip);
                    }, function(e) {
                        $(this).title.html(fixedDate);
                    });
                });
...
new MutationObserver(function(mutations) {
    ...
    document.querySelector('#question'), {         // monitor #questions for changes ---> it was 'questions'
        attributes: true,
        childList: true,
        characterData: true,
        subtree: true,
